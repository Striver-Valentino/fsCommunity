 ...
 */
@Service
public class UserActivateInterceptor implements HandlerInterceptor {
	...
	// 这个 UserActivateInterceptor 拦截器，拦截的 是 那些 需要 登陆 并且激活用户  才能 访问 的 请求。
	@Override
	public boolean preHandle(HttpServletRequest request,
			HttpServletResponse response, Object handler) throws Exception {
		
		System.out.println("UserActivateInterceptor 的 preHandle");
		...
		if(ticket != null){
			...
			/**
			 * 检查登陆没问题后，再检查用户是否已激活 （其实上面的检查登陆可以不用写，
			 * 因为前面一个 拦截器 CheckLoginInterceptor 已经 检查过 登陆了，
			 * 如果没登陆，是不会 访问到  UserActivateInterceptor 拦截器的）
			 */
			if(user.getStatus() != 0){
				// 状态不是0，说明没有激活，跳转到激活页面
				
				response.sendRedirect("/user/initUserIndex");
				return false;
			}
